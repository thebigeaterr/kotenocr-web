...
* =============================================================================
*/
function wd(e){let t=e.width,n=e.height,r=t,i=new Uint8ClampedArray(n*r*4);for(let a=0;a<r;a++){let r=t-1-a;for(let o=0;o<n;o++){let s=(o*t+r)*4,c=(a*n+o)*4;i[c]=e.data[s],i[c+1]=e.data[s+1],i[c+2]=e.data[s+2],i[c+3]=e.data[s+3]}}return{data:i,width:n,height:r}}function Td(e,t,n){let r=e.width,i=e.height,a=e.data,o=new Uint8ClampedArray(t*n*4),s=r/t,c=i/n;for(let e=0;e<n;e++){let n=(e+.5)*c-.5,l=Math.floor(n),u=n-l;l<0&&(l=0,u=0);let d=l+1;l>=i-1&&(l=i-1,d=l,u=0);for(let n=0;n<t;n++){let i=(n+.5)*s-.5,c=Math.floor(i),f=i-c;c<0&&(c=0,f=0);let p=c+1;c>=r-1&&(c=r-1,p=c,f=0);let m=(1-f)*(1-u),h=f*(1-u),g=(1-f)*u,_=f*u,v=(l*r+c)*4,y=(l*r+p)*4,b=(d*r+c)*4,x=(d*r+p)*4,S=(e*t+n)*4;for(let e=0;e<4;e++)o[S+e]=Math.round(m*a[v+e]+h*a[y+e]+g*a[b+e]+_*a[x+e])}}return{data:o,width:t,height:n}}function Ed(e){return e=Math.abs(e),e<1?(1.5*e-2.5)*e*e+1:e<2?(((e-5)*e+8)*e-4)*-.5:0}function Dd(e,t){let n=e/t,r=Math.max(n,1),i=2*r,a=Math.ceil(i)*2+1,o=new Int32Array(t*2),s=new Float64Array(t*a),c=1/r;for(let r=0;r<t;r++){let t=(r+.5)*n,l=Math.floor(t-i+.5);l<0&&(l=0);let u=Math.floor(t+i+.5);u>e&&(u=e),u-=l;let d=r*a,f=0;for(let e=0;e<u;e++){let n=Ed((e+l-t+.5)*c);s[d+e]=n,f+=n}if(f!==0)for(let e=0;e<u;e++)s[d+e]/=f;o[r*2]=l,o[r*2+1]=u}return{bounds:o,kk:s,ksize:a}}function Od(e,t){let{bounds:n,kk:r,ksize:i}=Dd(e.width,t),a=new Uint8ClampedArray(t*e.height*4);for(let o=0;o<e.height;o++){let s=o*e.width;for(let c=0;c<t;c++){let l=n[c*2],u=n[c*2+1],d=c*i,f=0,p=0,m=0,h=0;for(let t=0;t<u;t++){let n=r[d+t],i=(s+l+t)*4;f+=n*e.data[i],p+=n*e.data[i+1],m+=n*e.data[i+2],h+=n*e.data[i+3]}let g=(o*t+c)*4;a[g]=Math.round(f),a[g+1]=Math.round(p),a[g+2]=Math.round(m),a[g+3]=Math.round(h)}}return{data:a,width:t,height:e.height}}function kd(e,t){let{bounds:n,kk:r,ksize:i}=Dd(e.height,t),a=new Uint8ClampedArray(e.width*t*4);for(let o=0;o<t;o++){let t=n[o*2],s=n[o*2+1],c=o*i;for(let n=0;n<e.width;n++){let i=0,l=0,u=0,d=0;for(let a=0;a<s;a++){let o=r[c+a],s=((t+a)*e.width+n)*4;i+=o*e.data[s],l+=o*e.data[s+1],u+=o*e.data[s+2],d+=o*e.data[s+3]}let f=(o*e.width+n)*4;a[f]=Math.round(i),a[f+1]=Math.round(l),a[f+2]=Math.round(u),a[f+3]=Math.round(d)}}return{data:a,width:e.width,height:t}}function Ad(e,t,n){return kd(Od(e,t),n)}const jd=[`text_block`,`line_main`,`line_caption`,`line_ad`,`line_note`,`line_note_tochu`,`block_fig`,`block_ad`,`block_pillar`,`block_folio`,`block_rubi`,`block_chart`,`block_eqn`,`block_cfm`,`block_eng`,`block_table`,`line_title`],Md=[.485,.456,.406],Nd=[.229,.224,.225],Pd=[{key:30,file:`parseq30.onnx`,width:256},{key:50,file:`parseq50.onnx`,width:384},{key:100,file:`parseq100.onnx`,width:768}];function Fd(e,t){return new OffscreenCanvas(e,t)}var Id=class{ort;base;deim;rec=new Map;recWidth=new Map;charset=[];cascade=!0;constructor(e,t){this.ort=e,this.base=t}setCascade(e){this.cascade=e}async load(e){e({stage:`charset`,message:`文字セット読込中`});let t=await(await fetch(this.base+`models/charset.json`)).json();if(this.charset=Array.from(t.charset),this.charset.length!==7141)throw Error(`charset length ${this.charset.length} != 7141`);let n={executionProviders:[`wasm`],graphOptimizationLevel:`all`};e({stage:`model`,message:`レイアウト用プログラムを読み込み中`}),this.deim=await this.ort.InferenceSession.create(this.base+`models/deim.onnx`,n);for(let t of Pd)e({stage:`model`,message:`文字認識用プログラムを読み込み中（${t.key}字）`}),this.rec.set(t.key,await this.ort.InferenceSession.create(this.base+`models/`+t.file,n)),this.recWidth.set(t.key,t.width)}async detect(e){let t=e.width,n=e.height,r=Math.max(t,n),i=Fd(r,r).getContext(`2d`);i.fillStyle=`black`,i.fillRect(0,0,r,r),i.drawImage(e,0,0);let a=Ad({data:i.getImageData(0,0,r,r).data,width:r,height:r},800,800).data,o=800*800,s=new Float32Array(3*o);for(let e=0;e<o;e++){let t=a[e*4]/255,n=a[e*4+1]/255,r=a[e*4+2]/255;s[e]=(t-Md[0])/Nd[0],s[o+e]=(n-Md[1])/Nd[1],s[2*o+e]=(r-Md[2])/Nd[2]}let c=this.deim.inputNames,l={};l[c[0]]=new this.ort.Tensor(`float32`,s,[1,3,800,800]),l[c[1]]=new this.ort.Tensor(`int64`,BigInt64Array.from([BigInt(800),BigInt(800)]),[1,2]);let u=await this.deim.run(l),d=this.deim.outputNames,f=Bd(u[d[0]].data),p=Bd(u[d[1]].data),m=Bd(u[d[2]].data),h=d.length>=4?Bd(u[d[3]].data):null,g=r/800,_=[];for(let e=0;e<m.length;e++){if(m[e]<=.25)continue;let t=Math.trunc(p[e*4]*g),n=Math.trunc(p[e*4+1]*g),i=Math.trunc(p[e*4+2]*g),a=Math.trunc(p[e*4+3]*g);if(t=zd(t,0,r),i=zd(i,0,r),n=zd(n,0,r),a=zd(a,0,r),i-t<1||a-n<1)continue;let o=Math.round(f[e])-1,s=jd[o]??`cls${o}`;_.push({x1:t,y1:n,x2:i,y2:a,classIndex:o,className:s,conf:m[e],charCount:h?h[e]:100,vertical:a-n>i-t})}let v=_.filter(e=>e.className.startsWith(`line`));return v.length===0&&(v=_),Rd(v,.6)}order(e){if(e.length<=1)return{ordered:e,vertical:e[0]?.vertical??!0};let t=e.filter(e=>e.vertical).length/e.length>.5,n=e=>{let t=[...e].sort((e,t)=>e-t);return t[t.length>>1]},r=e.map(e=>({l:e,cx:(e.x1+e.x2)/2,cy:(e.y1+e.y2)/2,w:e.x2-e.x1,h:e.y2-e.y1})),i=[];if(t){let e=Math.max(8,n(r.map(e=>e.w))*.6),t=[];for(let n of[...r].sort((e,t)=>t.cx-e.cx)){let r=t.find(t=>Math.abs(t[0].cx-n.cx)<=e);r?r.push(n):t.push([n])}for(let e of t){e.sort((e,t)=>e.cy-t.cy);for(let t of e)i.push(t.l)}}else{let e=Math.max(6,n(r.map(e=>e.h))*.6),t=[];for(let n of[...r].sort((e,t)=>e.cy-t.cy)){let r=t.find(t=>Math.abs(t[0].cy-n.cy)<=e);r?r.push(n):t.push([n])}for(let e of t){e.sort((e,t)=>e.cx-t.cx);for(let t of e)i.push(t.l)}}return{ordered:i,vertical:t}}async recOne(e,t,n){let r=this.rec.get(e),i=this.recWidth.get(e),a=n.x2-n.x1,o=n.y2-n.y1;if(a<1||o<1)return``;let s=Fd(a,o).getContext(`2d`);s.drawImage(t,n.x1,n.y1,a,o,0,0,a,o);let c={data:s.getImageData(0,0,a,o).data,width:a,height:o};o>a&&(c=wd(c));let l=Td(c,i,24).data,u=i*24,d=new Float32Array(3*u);for(let e=0;e<u;e++)d[e]=l[e*4+2]/127.5-1,d[u+e]=l[e*4+1]/127.5-1,d[2*u+e]=l[e*4]/127.5-1;let f={};f[r.inputNames[0]]=new this.ort.Tensor(`float32`,d,[1,3,24,i]);let p=(await r.run(f))[r.outputNames[0]],m=Bd(p.data),h=p.dims,g=h[1],_=h[2],v=``;for(let e=0;e<g;e++){let t=0,n=-1/0,r=e*_;for(let e=0;e<_;e++){let i=m[r+e];i>n&&(n=i,t=e)}if(t===0)break;v+=this.charset[t-1]??``}return v}async recognize(e,t){if(!this.cascade)return this.recOne(100,e,t);let n=Math.round(t.charCount),r=n===3?30:n===2?50:100;if(r===30){let n=await this.recOne(30,e,t);if(n.length<25)return n;r=50}if(r===50){let n=await this.recOne(50,e,t);if(n.length<45)return n;r=100}return this.recOne(100,e,t)}async run(e,t){let n=globalThis.performance?.now?.()??0;t({stage:`detect`,message:`レイアウト検出中`});let r=await this.detect(e),{ordered:i,vertical:a}=this.order(r),o=[];for(let n=0;n<i.length;n++)t({stage:`recognize`,done:n,total:i.length,message:`文字認識中 ${n+1}/${i.length}`}),o.push({box:i[n],text:await this.recognize(e,i[n])});return{text:o.map(e=>e.text).join(`
`),lines:o,vertical:a,width:e.width,height:e.height,ms:(globalThis.performance?.now?.()??0)-n}}};function Ld(e,t){let n=Math.max(e.x1,t.x1),r=Math.max(e.y1,t.y1),i=Math.min(e.x2,t.x2),a=Math.min(e.y2,t.y2),o=Math.max(0,i-n)*Math.max(0,a-r);return o<=0?0:o/((e.x2-e.x1)*(e.y2-e.y1)+(t.x2-t.x1)*(t.y2-t.y1)-o)}function Rd(e,t){let n=[];for(let r of e)n.some(e=>Ld(e,r)>t)||n.push(r);return n}function zd(e,t,n){return e<t?t:e>n?n:e}function Bd(e){if(e instanceof Float32Array)return e;if(e instanceof BigInt64Array){let t=Array(e.length);for(let n=0;n<e.length;n++)t[n]=Number(e[n]);return t}let t=e,n=Array(t.length);for(let e=0;e<t.length;e++)n[e]=Number(t[e]);return n}let Vd=null;function Hd(e){self.postMessage(e)}self.onmessage=async e=>{let t=e.data;try{if(t.type===`init`)T.wasm.numThreads=1,Vd=new Id(r,t.base),await Vd.load(e=>Hd({type:`progress`,...e})),Hd({type:`ready`});else if(t.type===`run`){if(!Vd)throw Error(`not initialized`);Vd.setCascade(t.cascade!==!1);let e=await createImageBitmap(t.blob),n=await Vd.run(e,e=>Hd({type:`progress`,...e}));e.close(),Hd({type:`result`,result:n,jobId:t.jobId})}}catch(e){Hd({type:`error`,message:e instanceof Error?`${e.message}`:String(e)})}};