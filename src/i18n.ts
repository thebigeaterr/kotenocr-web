...
export const T = {
  ...
  brand: { ja: 'セキュアOCR', en: 'Secure OCR' },
  ...
  loading: { ja: '画像処理プログラムを読み込み中…', en: 'Loading the image-processing program…' },
  loadingNote: {
    ja: '画像をPCの外にアップロードせず、あなたのPCの中だけで処理するため、最初に「画像処理プログラム」（約180MB）を読み込みます。読み込んだプログラムは、あなたのPC内（ブラウザのこのサイト専用の保存領域）に保存され、次回からは再ダウンロード不要です。',
    en: 'Because images are processed only on your PC (never uploaded), the image-processing program (~180MB) is loaded first. It is stored on your PC (in the browser\'s storage for this site), so no re-download is needed next time.',
  },
  loadingShort: { ja: '画像処理プログラムを読込中（約180MB・初回のみ）', en: 'Loading program (~180MB, first time only)' },
  copy: { ja: 'コピー', en: 'Copy' },
